...
async function renderQuestionsTab() {
  ...
  function buildEssayUI(referenceAnswer = "") {
    optionsBox.innerHTML = `
      <label>الإجابة النموذجية (مرجع اختياري)</label>
      <textarea id="essayRefAnswer" rows="4" placeholder="اكتب هنا الإجابة النموذجية لهذا السؤال (اختياري) - هتظهر للطالب كمرجع مع نتيجته بعد التصحيح">${esc(referenceAnswer)}</textarea>
      <p class="a-hint-text">السؤال المقالي بيتم تصحيحه يدوياً من الأدمن، والإجابة دي مجرد مرجع بيشوفه الطالب مع نتيجته، مش بيتم التصحيح التلقائي بيها.</p>`;
  }
  ...
  function updateFormForType(type, data = {}) {
    ...
    if (type === "true_false") {
      ...
      buildEssayUI(typeof data.correct_answer === "string" ? data.correct_answer : "");
    }
  }
  ...
  document
    ...
    .addEventListener("submit", async (e) => {
      ...
      if (type === "true_false") {
        ...
        payload.options = null;
        const refAnswer = (document.getElementById("essayRefAnswer")?.value || "").trim();
        payload.correct_answer = refAnswer || null;
        payload.blank_scores = null;
        payload.score = Number(document.getElementById("qScore").value) || 1;
      }
  ...
  if (!questions.length) {
    ...
    listEl.innerHTML = questions
      .map((q, idx) => {
        let answerLabel = "الإجابة الصحيحة:";
        let answerDisplay = "";
        if (q.type === "fill_in_the_blank") {
          ...
          answerDisplay = q.correct_answer
            ? q.correct_answer
            : "تصحيح يدوي (لم يتم إضافة إجابة نموذجية)";
        } else {
          answerDisplay = q.correct_answer;
        }
        ...
        <div class="a-question-card" data-id="${q.id}">
          ...
          <div class="a-question-card-body">
            ...
            <div class="a-question-answer-line"><b>${answerLabel}</b> ${esc(String(answerDisplay))}</div>
          </div>
        </div>`;
      })
      .join("");
