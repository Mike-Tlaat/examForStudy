...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderResultCard(attempt) {
    ...
    document.getElementById("resPercentage").textContent = `${scorePct}%`;
    ...
    resultCard.classList.add("has-review");
    const reviewSlot = document.getElementById("answersReviewSlot");
    ...
    if (reviewSlot) {
      buildReviewSectionHtml(attempt.exam_id, attempt.id)
        .then((html) => {
          if (html) reviewSlot.innerHTML = html;
        })
        .catch((err) => console.error("Error loading answer review:", err));
    }
  }
